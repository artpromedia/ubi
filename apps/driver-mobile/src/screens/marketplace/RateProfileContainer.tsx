...
import { marketplaceApi, type MpRatePreview, type MpRateProfile } from '../../api/marketplace';
...
type MpRateProfileDto = MpRateProfile & { scopeLabel?: string; componentsLine?: string; versionLine?: string };
...
const toMinor = (raw: string) => (parseInt(raw.replace(/\D/g, ''), 10) || 0) * 100;

export function RateProfileContainer() {
  const nav = useNavigation<{ goBack: () => void }>();
  const q = useQuery({ queryKey: ['mp', 'rateProfiles'], queryFn: marketplaceApi.rateProfiles });
  ...
  const previewM = useMutation({
    mutationFn: (inputs: { perKmMinor: number; minimumTripFareMinor: number }) => marketplaceApi.ratePreview({
      cityId: profile!.cityId, service: profile!.service, vehicleClass: profile!.vehicleClass,
      perKmMinor: inputs.perKmMinor, minimumTripFareMinor: inputs.minimumTripFareMinor,
      exampleDistanceMeters: 10_000,
    }),
    onSuccess: (p, inputs) => { setPreview(p); previewedFor.current = inputs.perKmMinor + ':' + inputs.minimumTripFareMinor; },
  });
  const refreshPreview = (rateRaw: string, minRaw: string) => previewM.mutate({ perKmMinor: toMinor(rateRaw), minimumTripFareMinor: toMinor(minRaw) });
  ...
  const save = useMutation({
    mutationFn: () => marketplaceApi.saveRateProfile({
      cityId: profile!.cityId, service: profile!.service, vehicleClass: profile!.vehicleClass,
      perKmMinor: toMinor(rateValue), minimumTripFareMinor: toMinor(minValue),
    }),
    onSuccess: (saved) => { setSaveError(null); track('driver_mp_rates_saved', { profileId: saved.profileId, version: saved.version }); void q.refetch(); },
    onError: (e) => { if (e instanceof ApiError && e.code === 'rate_profile_out_of_bounds') setSaveError(e.message); else setSaveError((e as Error).message); },
  });
  if (q.isError) {
    ...
    return (
      <Screen title="My rates" onBack={nav.goBack} bg="bg2">
        <Banner tone={notConfigured ? 'neutral' : 'error'} title={notConfigured ? 'Not available here yet' : 'Couldn’t load your rates'} body={(q.error as Error).message} />
      </Screen>
    );
  }
  if (!profile) return <Screen title="My rates" onBack={nav.goBack} bg="bg2"><Skeleton height={140} /><Skeleton height={180} /></Screen>;
  const inputsKey = toMinor(rateValue) + ':' + toMinor(minValue);
  return (
    <RateProfileScreen
      scopeLabel={profile.scopeLabel ?? profile.cityId + ' · ' + profile.service + ' · ' + profile.vehicleClass}
      rateValue={rateValue}
      ...
      preview={preview ? { stale: previewedFor.current !== inputsKey, rows: preview.rows, disclaimer: preview.disclaimer } : null}
      onRefreshPreview={() => refreshPreview(rateValue, minValue)}
      saveError={saveError}
      versionLine={profile.versionLine ?? 'Changes apply to future calculations only — live offers and won jobs keep their agreed amounts. Currently saved as v' + profile.version + '.'}
