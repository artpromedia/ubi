...
import type { RequestsStackParamList, WalletHoldsParams } from '../../navigation/routes';
import { marketplaceApi, type MpPreset } from '../../api/marketplace';
import { useMotionGate } from '../../lib/motion';
import { RequestDetailScreen, type EligibilityReason } from './RequestDetailScreen';
...
export function RequestDetailContainer() {
  ...
  const toWallet = (shortfall: { title: string; detail: string } | null) => {
    const p: WalletHoldsParams = {
      ...(shortfall ? { shortfall } : {}),
      ...(stillOpen ? { returnTo: { requestId: params.requestId, label: 'Back to request · still open ' + mmss(view!.item.expiresAt) } } : {}),
    };
  ...
  const submit = useMutation({
    mutationFn: (preset: MpPreset) => marketplaceApi.submitBid({
  ...
  const withdraw = useMutation({
    ...
    onSuccess: (b) => { track('driver_mp_bid_withdrawn', { bidId: b.bidId }); void q.refetch(); },
    onError: () => { void q.refetch(); },
  });

  if (q.isError) {
    const closed = q.error instanceof ApiError && q.error.code === 'request_closed';
    return (
      <Screen title="Request" onBack={nav.goBack} bg="bg2">
        <Banner tone={closed ? 'neutral' : 'error'} title={closed ? 'This request has closed' : 'Couldn’t load this request'} body={(q.error as Error).message} />
        <Button label="Back to requests" kind="secondary" onPress={nav.goBack} />
      </Screen>
    );
  }
  if (!view) return <Screen title="Request" onBack={nav.goBack} bg="bg2"><Skeleton height={120} /><Skeleton height={90} /><Skeleton height={220} /></Screen>;
  ...
  const blocked: EligibilityReason[] | null = !view.eligibility.eligible
    ...
    : gate.motion !== 'parked_confirmed'
      ? [gate.motion === 'moving'
        ? { code: 'NOT_STATIONARY', title: 'You’re moving', detail: 'Offer controls stay hidden until you’re safely parked. The request stays in your feed.' }
        : { code: 'LOCATION_STALE', title: 'Location signal stale', detail: 'Bidding is paused until GPS recovers or you confirm you’re safely parked.' }]
      : null;

  const myBidDto = view.myBid && LIVE_BID.has(view.myBid.state) ? view.myBid : null;
  ...
  return (
    <RequestDetailScreen
      ...
      presets={view.presets.map((c) => ({ key: c.key, title: c.title, feeNetLabel: c.feeNetLabel, affordable: c.affordable, shortfallLabel: c.shortfallLabel, emphasized: c.emphasized }))}
      onBid={onPreset}
      ...
      stationary={false}
      onCustom={() => {}}
      onSkip={() => { track('driver_mp_request_skipped', { requestId: params.requestId }); nav.goBack(); }}
      onTopUp={() => {
        const short = view.presets.find((c) => !c.affordable && c.shortfallLabel);
        toWallet(short?.shortfallLabel ? { title: 'Top-up needed to place this offer', detail: short.shortfallLabel } : null);
      }}
      blocked={blocked}
      myBid={myBidDto && !revising ? {
