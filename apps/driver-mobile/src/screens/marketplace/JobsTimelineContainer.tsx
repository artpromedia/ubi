...
export const jobCard = (c: MpDriverJob): JobCard => ({
  ...
  title: (c.service === 'delivery' ? 'Delivery' : 'Ride') + ' · ' + formatMinor(c.fareMinor) + ' agreed',
  fareMinor: c.fareMinor,
  feeLine: 'Fee ' + formatMinor(c.commissionMinor) + ' · debited at selection' + (c.receiptId ? ' · ' + c.receiptId : ''),
  feeReceiptId: c.receiptId ?? '',
  detail: c.pickupWindow ? 'Pickup commitment ' + Math.round(c.pickupWindow.earliestSec / 60) + '–' + Math.round(c.pickupWindow.latestSec / 60) + ' min · rider sees live updates' : '',
  remainingLabel: null,
});
...
export function JobsTimelineContainer() {
  ...
  const toTrip = (tripId: string | null | undefined, screen: 'Navigate' | 'InTrip') => {
    if (!tripId) return;
  ...
  return (
    <JobsTimelineScreen
      ...
      onContinueCurrent={() => toTrip(view.current?.executionRef?.id ?? null, 'InTrip')}
      onBack={nav.goBack}
    />
  );
}
