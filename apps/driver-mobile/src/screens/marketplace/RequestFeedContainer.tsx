 ...
 */
export const holdStateOrSafe = (
  v: unknown,
  safe: "held" | "release_pending",
...
export const bidRow = (b: MpBidDto): MyBid | null => {
  ...
  return {
    ...
    holdState: holdStateOrSafe(
      b.holdState,
      status === "awaiting" ? "held" : "release_pending",
    ),
    holdDetail:
      b.holdDetail ??
      (status === "awaiting" ? "closes " + mmss(b.expiresAt) : ""),
  };
};
...
export function RequestFeedContainer() {
  ...
  const bidsQ = useQuery({
    queryKey: ["mp", "bids", "mine"],
    queryFn: marketplaceApi.myBids,
    refetchInterval: tab === "myBids" ? 5_000 : 30_000,
  });
  ...
  return (
    <RequestFeedScreen
      ...
      requests={requests}
      onOpen={(requestId) => {
        track("driver_mp_request_opened", { requestId });
        nav.navigate("Detail", { requestId });
      }}
      deferredPrompt={
        gate.motion === "moving" ? (feedQ.data?.deferredPrompt ?? null) : null
      }
      ...
      quickLinks={[
        ...
        },
        {
          key: "wallet",
          label: "Wallet holds",
          onPress: () => nav.navigate("WalletHolds"),
        },
        {
          key: "rates",
          label: "My rates",
