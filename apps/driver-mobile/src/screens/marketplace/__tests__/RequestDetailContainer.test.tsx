  ...
  presets: [
    {
      ...
      commissionMinor: money(280_00),
      netMinor: money(2520_00),
      title: "Offer ₦2,800",
...
describe("RequestDetailContainer", () => {
  ...
  it("sends driver-view.currentClaimId as dependsOnClaimId on a next-slot bid", async () => {
    ...
    installFixtures(async ({ method, path, body }) => {
      if (method === "GET" && path === "/v1/mp/requests/req_next_1/driver-view")
    ...
    fireEvent.press(
      await screen.findByTestId(
        dynamicTestId(TEST_IDS.mp.driver.detail.preset, 0),
      ),
    );
    await waitFor(() => expect(bidBodies).toHaveLength(1));
    expect(
      (bidBodies[0] as { dependsOnClaimId?: string }).dependsOnClaimId,
    ).toBe("clm_cur_77");
  ...
  it("surfaces a 4xx rejection with the server message instead of a silent refetch", async () => {
      ...
      if (method === "POST" && path === "/v1/mp/bids") {
        return {
          ...
          json: {
            ...
            message: "dependsOnClaimId must name your current claim",
          },
        };
      }
      return undefined;
    ...
    expect(
      await screen.findByText("dependsOnClaimId must name your current claim"),
    ).toBeTruthy();
    view.unmount(); // stop the container's 5s refetch interval
  });
});
