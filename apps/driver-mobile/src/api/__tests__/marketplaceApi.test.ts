    ...
    installFixtures(async ({ method, path }) => {
      if (method === "GET" && path.startsWith("/v1/wallet/mp/overview")) {
        ...
        return {
          status: 200,
          json: {
            clearedMinor: { amountMinor: 0, currency: "NGN" },
            heldMinor: { amountMinor: 0, currency: "NGN" },
            spendableMinor: { amountMinor: 0, currency: "NGN" },
            holds: [],
          },
        };
      }
...
describe("bidRejection (submit/revise 4xx surfacing)", () => {
  it("carries the server message verbatim for queue_dependency_invalid", () => {
    ...
    expect(bidRejection(e)).toEqual({
      title: "Your offer wasn’t placed",
      detail: "dependsOnClaimId must name your current claim",
    });
  });
  it("carries the server message for any other 4xx code", () => {
    const e = new ApiError(
      409,
      "version_conflict",
      "This request changed — review the new terms before bidding.",
    );
    expect(bidRejection(e).detail).toBe(
      "This request changed — review the new terms before bidding.",
    );
  });
  it("still yields an honest banner for non-API failures", () => {
    expect(bidRejection(new Error("Network request failed")).detail).toBe(
