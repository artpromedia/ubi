...
import type { RootStackParamList, MainTabParamList, IncentivesStackParamList, EarningsStackParamList, RequestsStackParamList } from './routes';
...
function RequestsNavigator({ navigation }: BottomTabScreenProps<MainTabParamList, 'Requests'>) {
  return (
    <MarketplaceGate featureName="Requests" onDismiss={() => navigation.navigate('Home')}>
...
function GatedWalletHolds({ navigation }: { navigation: { navigate: (s: 'Main') => void } }) {
  return <MarketplaceGate featureName="Wallet holds" onDismiss={() => navigation.navigate('Main')}><WalletHoldsContainer /></MarketplaceGate>;
}
function GatedRates({ navigation }: { navigation: { navigate: (s: 'Main') => void } }) {
  return <MarketplaceGate featureName="My rates" onDismiss={() => navigation.navigate('Main')}><RateProfileContainer /></MarketplaceGate>;
}
function GatedJobs({ navigation }: { navigation: { navigate: (s: 'Main') => void } }) {
  return <MarketplaceGate featureName="Your jobs" onDismiss={() => navigation.navigate('Main')}><JobsTimelineContainer /></MarketplaceGate>;
}
function MainTabs() {
  const t = useTheme();
  return (
