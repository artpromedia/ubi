...
const feeMinor = (grossMinor: number) =>
  Math.floor((grossMinor * 1_000 + 5_000) / 10_000);
const money = (amountMinor: number) => ({ amountMinor, currency: "NGN" });
const naira = (amountMinor: number) =>
  "₦" + Math.round(amountMinor / 100).toLocaleString("en-NG");

type MpDriverFixState = {
  bidsPolls: number;
  jobsPolls: number;
  topupClearPolls: number | null;
  clearedMinor: number;
  myBid: null | {
    bidId: string;
    amountMinor: number;
...
const fresh = (): MpDriverFixState => ({
  bidsPolls: 0,
  jobsPolls: 0,
  topupClearPolls: null,
  clearedMinor: 1000_00,
  myBid: null,
  topups: [],
});
...
const feedItems = () => [
  {
    requestId: "req_mp_201",
    revision: 1,
    service: "ride",
    title: "Lekki Phase 1 → Victoria Island",
    meta: "Ride · Economy · 8.4 km · ~22 min · pickup 1.2 km from you",
    askedMinor: NGN(2800),
  ...
  {
    requestId: "req_mp_202",
    revision: 1,
    service: "delivery",
    title: "Ikoyi → Yaba",
    meta: "Delivery · Box · 6.1 km · ~19 min · pickup 0.8 km from you",
    askedMinor: NGN(1800),
    askedByLabel: "sender asks",
    capabilityBadge: "Box · up to 5 kg",
    expiresAt: iso(120_000),
  },
  {
    requestId: "req_mp_210",
    revision: 2,
    service: "ride",
    title: "Ajah → Epe",
    meta: "Ride · Economy · 24.8 km · ~41 min · pickup 9.4 km from you",
    askedMinor: NGN(6500),
    askedByLabel: "rider asks",
    capabilityBadge: null,
...
const activeHolds = () => {
  ...
  const holds = [
    {
      reservationId: "rsv_a",
      bidId: "bid_a",
      title: "Surulere → Ikeja · your offer " + naira(2800_00),
      amountMinor: money(HOLD_A),
      releaseCondition:
        "Releases if the requester picks another driver or the request closes",
      state: "active",
    },
    ...(bLost()
      ? []
      : [
          {
            reservationId: "rsv_b",
            bidId: "bid_b",
            title: "Ikeja → Maryland · offer lost",
            amountMinor: money(HOLD_B),
            releaseCondition:
              "Release pending · confirms when the requester’s choice is final",
            state: "active" as const,
          },
        ]),
    ...(s.myBid && myBidPhase(s) === "live"
      ? [
          {
            reservationId: "rsv_new",
            bidId: s.myBid.bidId,
            title:
              "Lekki Phase 1 → Victoria Island · your offer " +
              naira(s.myBid.amountMinor),
            amountMinor: money(feeMinor(s.myBid.amountMinor)),
            releaseCondition:
              "Releases if the requester picks another driver or the request closes",
            state: "active" as const,
          },
  ...
  return holds.map((h) => ({
    ...h,
    driverId: "u_drv_chinedu",
    commissionBps: 1000,
    baseMinor: money(h.amountMinor.amountMinor * 10),
...
const heldMinor = () =>
  activeHolds().reduce((sum, h) => sum + h.amountMinor.amountMinor, 0);
    ...
    return [
      {
        ...
        title: "You’re moving",
        detail:
          "Park safely, then confirm — the request stays in your feed and nothing counts against you.",
      },
    ];
  if (m === "stale_location")
    return [
      {
        ...
        detail:
          "We can’t estimate your pickup time. Bidding resumes when GPS recovers or you confirm you’re parked.",
      },
    ];
  return [];
};
...
const blockedReasons = [
  {
    code: "OUTSIDE_RADIUS",
    title: "Too far from the pickup",
    detail:
      "This request is outside your 5 km search envelope. Head toward Ajah to see it become biddable.",
  },
  {
    code: "PICKUP_ETA_TOO_LONG",
    title: "Pickup would take too long",
    detail:
      "Your estimated arrival is over the request’s 15 min pickup limit. Closer requests appear in your feed.",
  },
  {
    code: "LOCATION_STALE",
    title: "Location signal stale",
  ...
  {
    code: "SLOT_FULL",
    title: "Both job slots are taken",
    detail:
      "You have a current trip and a queued next job. Bidding reopens when one completes.",
  },
  {
    code: "INSUFFICIENT_SPENDABLE",
    title: "Not enough spendable balance",
    detail:
      "Every offer reserves its 10% fee first. Top up in Wallet, then come back — this request stays open.",
  },
];
...
const myBidPhase = (
  s: MpDriverFixState,
): "live" | "lost_release_pending" | "lost_released" | "withdrawn" => {
  ...
  return {
    ...
    state:
      phase === "withdrawn"
        ...
        : phase === "live"
          ...
          : "lost",
    driverId: "u_drv_chinedu",
    amountMinor: money(b.amountMinor),
    commissionMinor: money(fee),
    netMinor: money(b.amountMinor - fee),
    slot: "current",
    dependsOnClaimId: null,
    reservationId: "rsv_new",
    expiresAt: iso(125_000),
    ...
    holdDetail:
      phase === "live"
        ...
        : phase === "lost_released"
          ? "confirmed just now"
          : "your money returns once the choice is final",
  };
};
...
const myBidsList = () => {
  ...
  const rows = [
    {
      bidId: "bid_a",
      requestId: "req_mp_190",
      requestRevision: 1,
      bidVersion: 1,
      state: "submitted",
      driverId: "u_drv_chinedu",
      amountMinor: money(2800_00),
      commissionMinor: money(HOLD_A),
      netMinor: money(2800_00 - HOLD_A),
      slot: "current",
      dependsOnClaimId: null,
      reservationId: "rsv_a",
      expiresAt: iso(125_000),
      createdAt: iso(-240_000),
      title: "Surulere → Ikeja",
      holdState: "held",
      holdDetail: "closes 2:05",
    },
    {
      bidId: "bid_b",
      requestId: "req_mp_185",
      requestRevision: 1,
      bidVersion: 2,
      state: "lost",
      driverId: "u_drv_chinedu",
      amountMinor: money(2200_00),
      commissionMinor: money(HOLD_B),
      netMinor: money(2200_00 - HOLD_B),
      slot: "current",
      dependsOnClaimId: null,
      reservationId: "rsv_b",
      ...
      holdState: bLost() ? "released" : "release_pending",
      holdDetail: bLost()
        ? "confirmed 9:40"
        : "your money returns once the choice is final",
    },
    {
      bidId: "bid_c",
      requestId: "req_mp_170",
      requestRevision: 1,
      bidVersion: 1,
      state: "expired",
      driverId: "u_drv_chinedu",
      amountMinor: money(3000_00),
      commissionMinor: money(300_00),
      netMinor: money(2700_00),
      slot: "current",
      dependsOnClaimId: null,
      reservationId: "rsv_c",
      expiresAt: iso(-3_600_000),
      createdAt: iso(-4_000_000),
      title: "Yaba → Ebute Metta",
      holdState: "released",
      holdDetail: "confirmed 8:12",
    },
  ];
...
const rateProfile = () => ({
  ...
  currency: "NGN",
  perKmMinor: 300_00,
  minimumTripFareMinor: 700_00,
  components: {
    perMinuteMinor: null,
    pickupPerKmMinor: null,
    handlingMinor: null,
  },
  createdAt: iso(-86_400_000),
  scopeLabel: "Lagos · Economy · sedan",
  componentsLine: "Off · not configured",
  versionLine:
...
const ratePreview = (perKmMinor: number, minimumTripFareMinor: number) => {
  ...
  return {
    ...
    rows: [
      {
        label:
          ...
          naira(perKmMinor) +
          "/km" +
          (floorAdjusted ? " · floor applied" : ""),
        value: naira(gross),
        tone: exceedsCeiling ? "errorInk" : undefined,
      },
    ...
    disclaimer: exceedsCeiling
      ? "Above the " +
        naira(RATE_BOUNDS.perKmMax) +
        "/km ceiling for Lagos Economy — requests will clip your calculated offer to their limit. Before fuel and operating costs."
...
export async function marketplaceFixtures(i: FixtureInput) {
  ...
  if (i.method === "GET" && i.path.startsWith("/v1/mp/feed")) {
    return ok({
      ...
      availabilityEpoch: 7,
      deferredPrompt:
        currentMotion() === "moving" ? "1 request near your drop-off" : null,
    });
  }
  // ── Driver view (D02/D03/D10) ──
  if (
    i.method === "GET" &&
    /^\/v1\/mp\/requests\/req_mp_210\/driver-view$/.test(i.path)
  ) {
  ...
  if (
    i.method === "GET" &&
    /^\/v1\/mp\/requests\/(req_mp_201|req_mp_202)\/driver-view$/.test(i.path)
  ) {
    ...
    if (reasons.length)
      return ok({
        item,
        eligibility: eligibility({ eligible: false, reasons, slot: null }),
        presets: [],
        profileLine: null,
        ceilingNotice: null,
        myBid: null,
  ...
  if (
    i.method === "GET" &&
    /^\/v1\/mp\/requests\/[^/]+\/driver-view$/.test(i.path)
  ) {
    return {
      status: 409,
      json: {
        code: "request_closed",
        message: "This request has closed. Your feed refreshes automatically.",
      },
    };
  }
  // ── Bids (D02 → D03) ──
      ...
      return {
        ...
        json: {
          ...
          message:
            ...
            naira(fee - spend) +
            " to place this offer.",
          details: { shortfallMinor: money(fee - spend) },
        },
      };
    }
    s.myBid = {
      ...
      amountMinor: gross,
      version: 1,
      placedPolls: s.bidsPolls,
      state: "live",
    };
    return { status: 201, json: bidBody(s) };
  }
  if (i.method === "POST" && /^\/v1\/mp\/bids\/bid_new\/revise$/.test(i.path)) {
    if (!s.myBid || myBidPhase(s) !== "live")
      return {
        status: 409,
        json: {
          code: "bid_not_live",
          message:
            "This offer is no longer live. The request view has been refreshed.",
        },
      };
    const body = i.body as { amountMinor?: { amountMinor: number } };
      ...
      return {
        ...
        json: {
          ...
          message:
            ...
            " needs " +
            naira(delta) +
            " more spendable — you have " +
            naira(spendableMinor()) +
            ". Top up " +
            naira(delta - spendableMinor()) +
            " to raise this offer.",
          details: { shortfallMinor: money(delta - spendableMinor()) },
        },
      };
    }
  ...
  if (
    i.method === "POST" &&
    /^\/v1\/mp\/bids\/bid_new\/withdraw$/.test(i.path)
  ) {
    if (!s.myBid || myBidPhase(s) !== "live")
      return {
        status: 409,
        json: {
          code: "bid_not_live",
          message: "This offer is no longer live.",
        },
      };
    ...
    return ok({
      ...
      holdDetail: "your money returns shortly",
    });
  }
  ...
  // ── Rate profiles (D09) ──
  if (i.method === "GET" && i.path === "/v1/mp/rate-profiles")
    return ok({ profiles: [rateProfile()] });
  if (i.method === "PUT" && i.path === "/v1/mp/rate-profiles") {
    const b = i.body as { perKmMinor: number; minimumTripFareMinor: number };
    if (
      b.perKmMinor < RATE_BOUNDS.perKmMin ||
      b.perKmMinor > RATE_BOUNDS.perKmMax
    ) {
      return {
        status: 422,
        json: {
          code: "rate_profile_out_of_bounds",
          message:
            "Per-km rate must be between " +
            naira(RATE_BOUNDS.perKmMin) +
            " and " +
            naira(RATE_BOUNDS.perKmMax) +
            " for Lagos · Economy. Your current v" +
            profileVersion +
            " stays active.",
        },
      };
    ...
    if (
      b.minimumTripFareMinor < RATE_BOUNDS.minFareMin ||
      b.minimumTripFareMinor > RATE_BOUNDS.minFareMax
    ) {
      return {
        status: 422,
        json: {
          code: "rate_profile_out_of_bounds",
          message:
            "Minimum trip fare must be between " +
            naira(RATE_BOUNDS.minFareMin) +
            " and " +
            naira(RATE_BOUNDS.minFareMax) +
            " for Lagos · Economy. Your current v" +
            profileVersion +
            " stays active.",
        },
    ...
    return ok({
      ...rateProfile(),
      perKmMinor: b.perKmMinor,
      minimumTripFareMinor: b.minimumTripFareMinor,
    });
  }
  ...
  if (
    i.method === "GET" &&
    (i.path === "/v1/wallet/mp/overview" ||
      i.path.startsWith("/v1/wallet/mp/overview?"))
  ) {
    ...
    if (s.topupClearPolls !== null) {
      ...
      if (s.topupClearPolls >= 3) {
        for (const tp of s.topups)
          if (tp.state === "pending") {
            tp.state = "cleared";
            s.clearedMinor += tp.amountMinor;
          }
        s.topupClearPolls = null;
      }
    }
    return ok({
      clearedMinor: money(s.clearedMinor),
      heldMinor: money(heldMinor()),
      spendableMinor: money(spendableMinor()),
      holds: activeHolds(),
      topupPresets: ["₦500", "₦1,000", "₦2,000"],
      topups: s.topups.map(({ label, state }) => ({
        label: "Top-up · " + label,
        state,
      })),
    });
  ...
  if (i.method === "POST" && i.path === "/v1/wallet/mp/topups") {
    ...
    return {
      status: 202,
      json: {
        topups: s.topups.map(({ label: l, state }) => ({
          label: "Top-up · " + l,
          state,
        })),
      },
  ...
  if (i.method === "GET" && i.path === "/v1/mp/driver/jobs") {
    ...
    const next = {
      ...
      state: "queued",
      fareMinor: NGN(2800),
      commissionMinor: money(feeMinor(2800_00)),
      receiptId: "rcpt_mp_00291",
      executionRef: null,
      pickupWindow: { earliestSec: 720, latestSec: 1080, etaVersion: 3 },
    };
    if (s.jobsPolls >= 7) {
    ...
    return ok({
      current: {
        ...
        state: "in trip",
        fareMinor: NGN(3200),
        commissionMinor: money(feeMinor(3200_00)),
