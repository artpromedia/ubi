...
const policy: MarketplacePolicy = {
  ...
  fareBounds: {
    'ride:go': { absoluteFloorMinor: 120_000, costFloorMinor: 90_000, floorBpsOfSuggested: 6_000, ceilingBpsOfSuggested: 15_000 },
    'delivery:moto': { absoluteFloorMinor: 80_000, costFloorMinor: 60_000, floorBpsOfSuggested: 6_000, ceilingBpsOfSuggested: 15_000 },
  },
  searchEnvelope: { initialRadiusMeters: 2_000, maxRadiusMeters: 6_000, initialPickupEtaSec: 360, maxPickupEtaSec: 900, expandAfterSec: 45, minOffersBeforeExpand: 3, expansionSteps: 3 },
  stationary: { minDwellSec: 90, maxSpeedMps: 1.5, maxLocationAgeSec: 30, maxAccuracyMeters: 50, motionCloseSec: 20 },
  finishingTrip: { maxRemainingSec: 480, completionBufferSec: 120, uncertaintyBufferSec: 60, corridorMaxBearingDeltaDeg: 45 },
  bids: { bidExpirySec: 120, requestExpirySec: 600, revisionCooldownSec: 15, maxLiveBidsPerDriver: 3, maxOpenRequestsPerRequester: 2 },
  queue: { pickupWindowToleranceSec: 300 },
  rateProfileBounds: { go: { maxPerKmMinor: 50_000, maxMinimumTripFareMinor: 200_000 } },
};
const configured: CityConfigView = { cityId: 'lagos', version: 7, currency: 'NGN', currencyFractionDigits: 2, marketplace: policy };
...
describe('toMonitorRow', () => {
  ...
  it('drops terminal states and dashes missing fields', () => {
    ...
    const bare = toMonitorRow({ requestId: 'r', state: 'open', service: 'ride', cityId: 'lagos' });
...
describe('monitorStats', () => {
  it('counts states and warns on no-bid requests', () => {
    const rows = [
      { requestId: 'a', route: '', service: '', asked: '', bids: 0, reach: '', envelope: '', state: 'open' as const },
      { requestId: 'b', route: '', service: '', asked: '', bids: 0, reach: '', envelope: '', state: 'no_bids' as const },
      { requestId: 'c', route: '', service: '', asked: '', bids: 1, reach: '', envelope: '', state: 'awarded' as const },
    ];
    const stats = monitorStats(rows);
    expect(stats[0]).toMatchObject({ label: 'Open requests', value: '2', tone: 'warn' });
...
describe('timeline mapping', () => {
  it('tones events by type and keeps them append-only in order', () => {
    ...
    const view = toTimelineEvents({
      ...
      policyVersion: 3,
      events: [
        { at: '2026-09-20T10:02:11Z', type: 'mp.request.published', detail: 'v1' },
        { at: '2026-09-20T10:07:19Z', type: 'mp.award.confirmed', detail: 'won' },
      ],
    });
...
describe('policy fields — fail closed', () => {
  ...
  it('marks a missing marketplace block invalid and blocks publish', () => {
    const bare: CityConfigView = { cityId: 'kano', version: 1, currency: 'NGN', currencyFractionDigits: 2 };
  ...
  it('marks an unconfigured floor invalid and blocks publish', () => {
    const broken: CityConfigView = {
      ...configured,
      marketplace: { ...policy, fareBounds: { 'ride:go': { absoluteFloorMinor: 0, costFloorMinor: 0, floorBpsOfSuggested: 0, ceilingBpsOfSuggested: 15_000 } } },
    };
  ...
  it('blocks publish on an empty fareBounds record', () => {
    const empty: CityConfigView = { ...configured, marketplace: { ...policy, fareBounds: {} } };
...
describe('envelopeLine', () => {
  it('renders km, minutes and step', () => {
    expect(envelopeLine({ step: 2, radiusMeters: 3_500, pickupEtaSec: 540 })).toBe('3.5 km · 9 min · step 2');
