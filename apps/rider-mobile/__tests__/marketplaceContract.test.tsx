...
jest.mock('../src/dev/fixtures/index', () => ({
  ...
  NGN: (major: number) => ({ amountMinor: Math.round(major * 100), currency: 'NGN' }),
}));
...
const wrap = (el: React.ReactElement) => {
  ...
  return render(<QueryClientProvider client={qc}><ThemeProvider defaultMode="light">{el}</ThemeProvider></QueryClientProvider>);
};

const offerEmeka = () => ({
  bidId: 'bid_emeka', bidVersion: 1, requestRevision: 1, amountMinor: NGN(2800), kind: 'immediate',
  driver: { displayName: 'Emeka Okafor', initials: 'EO', rating: '4.9', completedTrips: 2413, vehicle: 'Toyota Corolla · grey', plateMasked: 'LAG · 423 ··' },
  pickupLabel: 'Pickup in 4 min · 1.2 km away', pickupWindow: null, expiresAt: iso(90_000), withdrawn: false, whyRecommended: null,
  bookingFeeMinor: NGN(200), totalMinor: NGN(3000), deltaLabel: null,
});

const requestBody = () => ({
  requestId: 'req_mp_1', state: 'open', revision: 1, version: 1, service: 'ride', vehicleClass: 'standard', cityId: 'LOS', currency: 'NGN',
  requesterId: 'u_rider_1', quoteId: 'q_mp_1', requestedFareMinor: NGN(2800), suggestedFareMinor: NGN(2600), minimumFareMinor: NGN(2400), maximumFareMinor: NGN(3900),
  pickup: { label: 'Lekki', lat: 6.4, lng: 3.4 }, dropoff: { label: 'VI', lat: 6.42, lng: 3.42 }, delivery: null,
  searchEnvelope: { step: 0, radiusMeters: 3000, pickupEtaSec: 600 },
  policyVersion: 3, pricingVersion: 'px_2026_09_1', expiresAt: iso(150_000), createdAt: new Date().toISOString(), closeReason: null,
});

const confirmedAward = () => ({
  awardId: 'awd_mp_1', requestId: 'req_mp_1', bidId: 'bid_emeka', state: 'confirmed', requestVersion: 1, bidVersion: 1,
  driverId: 'u_drv_emeka', requesterId: 'u_rider_1', fareMinor: NGN(2800), commissionMinor: NGN(280), slot: 'current',
  executionRef: { service: 'ride', id: 'ride_mp_901' }, createdAt: new Date().toISOString(), resolvedAt: new Date().toISOString(),
});
...
describe('BidDetailContainer — select wrapper { award, pickupPin? } and executionRef handoff', () => {
  it('parses the wrapped 202 body, holds the one-time PIN and navigates with the real executionRef ride id', async () => {
    ...
    marketplaceApi.request.mockResolvedValue({ request: requestBody(), offers: [offerEmeka()], seq: 1 });
    // The saga confirmed synchronously: the WRAPPER carries the one-time pickupPin.
    marketplaceApi.select.mockResolvedValue({ award: confirmedAward(), pickupPin: '4831' });
  ...
  it('never navigates on a placeholder ride id when executionRef is absent', async () => {
    mockRouteParams = { requestId: 'req_mp_1', bidId: 'bid_emeka' };
    marketplaceApi.request.mockResolvedValue({ request: requestBody(), offers: [offerEmeka()], seq: 1 });
    marketplaceApi.select.mockResolvedValue({ award: { ...confirmedAward(), executionRef: null } });
    marketplaceApi.award.mockResolvedValue({ ...confirmedAward(), executionRef: null });
...
describe('FareEditorContainer — malformed quote money never renders NaN', () => {
  it('keeps the skeleton (no amount input, no NaN) when the envelope carries bare-integer money', async () => {
    mockRouteParams = { quoteParams: { service: 'ride', vehicleClass: 'standard', pickup: { label: 'A', lat: 1, lng: 2 }, dropoff: { label: 'B', lat: 3, lng: 4 } } };
    // Bare integers (old broken server shape) instead of contract Money objects.
    marketplaceApi.quote.mockResolvedValue({
      quoteId: 'q_bad', service: 'ride', vehicleClass: 'standard', cityId: 'LOS', currency: 'NGN',
      suggestedFareMinor: 260000, minimumFareMinor: 240000, maximumFareMinor: 390000,
      expiresAt: iso(120_000), pricingVersion: 'px', policyVersion: 3, breakdown: [],
    });
    ...
    await waitFor(() => expect(screen.queryByTestId(TEST_IDS.mp.rider.fare.amountInput)).toBeNull());
  ...
  it('seeds the editor from a valid contract Money quote', async () => {
    mockRouteParams = { quoteParams: { service: 'ride', vehicleClass: 'standard', pickup: { label: 'A', lat: 1, lng: 2 }, dropoff: { label: 'B', lat: 3, lng: 4 } } };
    marketplaceApi.quote.mockResolvedValue({
      quoteId: 'q_ok', service: 'ride', vehicleClass: 'standard', cityId: 'LOS', currency: 'NGN',
      suggestedFareMinor: NGN(2600), minimumFareMinor: NGN(2400), maximumFareMinor: NGN(3900),
      expiresAt: iso(120_000), pricingVersion: 'px', policyVersion: 3,
