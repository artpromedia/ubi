...
// and never sent to analytics. A driver account is refused with a plain
// explanation instead of a broken rider session.
...
export function OtpScreen() {
  ...
  const verify = useMutation({
    ...
    onSuccess: (outcome) => {
      ...
      // navigator. `reset` (unlike `navigate`) never bubbles to a parent
      // navigator, so it must be dispatched on the Root navigator itself —
      // resetting the Auth stack's own state to a "Main" route would fail,
      // since "Main" isn't part of AuthStackParamList.
    ...
    >
      <View style={{ gap: 12, paddingTop: 12 }}>
        ...
        {wrongApp ? (
          <Banner
            tone="warn"
            title="This is a driver account"
            body="Sign in with the UBI Driver app instead — this app is for riders."
          />
        ) : null}
        {verify.isError ? (
          <Banner
