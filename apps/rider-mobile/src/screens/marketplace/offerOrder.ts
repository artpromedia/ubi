...
export function mergeArrivalOrder(order: string[], offers: { bidId: string }[]): string[] {
  const seen = new Set(order);
  const next = order.slice();
  for (const o of offers) if (!seen.has(o.bidId)) { seen.add(o.bidId); next.push(o.bidId); }
 ...
 */
export function sortOfferIds(offers: MpOfferDto[], sort: 'price' | 'eta'): string[] {
  const idx = new Map(offers.map((o, i) => [o.bidId, i]));
  const byArrival = (a: MpOfferDto, b: MpOfferDto) => (idx.get(a.bidId) ?? 0) - (idx.get(b.bidId) ?? 0);
  const sorted = offers.slice().sort((a, b) => {
    if (sort === 'price') return a.amountMinor.amountMinor - b.amountMinor.amountMinor || byArrival(a, b);
    const ka = a.kind === 'immediate' ? 0 : 1;
    const kb = b.kind === 'immediate' ? 0 : 1;
    return ka - kb || (a.pickupWindow?.earliestSec ?? 0) - (b.pickupWindow?.earliestSec ?? 0) || byArrival(a, b);
  });
...
export function displayOrder(arrival: string[], sortedSnapshot: string[] | null): string[] {
