...
export function OfferInboxContainer() {
  ...
  useEffect(() => {
    const t = setInterval(() => setClock((c) => c + 1), 1_000);
  ...
  const offers: Offer[] = dtos.map((o) => ({
    ...
    driverName: o.driver.displayName,
    rating: o.driver.rating,
    trips: o.driver.completedTrips,
    ...
    deltaLabel:
      o.deltaLabel ??
      (o.amountMinor.amountMinor === r.requestedFareMinor.amountMinor
        ? null
        : o.amountMinor.amountMinor > r.requestedFareMinor.amountMinor
          ? "above your ask"
          : "below your ask"),
  ...
  const phase =
    r.state === "no_offers" || (r.state === "expired" && offers.length === 0)
      ? "no_offers"
  ...
  return (
    <OfferInboxScreen
      ...
      onSort={(s) => {
        setSort(s);
        setSortedIds(sortOfferIds(dtos, s));
