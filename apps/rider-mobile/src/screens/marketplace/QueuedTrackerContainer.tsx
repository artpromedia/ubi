...
export function QueuedTrackerContainer() {
  ...
  const q = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      if (
        v?.delayed?.reversal &&
        v.delayed.reversal.riderHold === "released" &&
        v.delayed.reversal.driverFee === "reversed"
      )
        return false;
      return 5_000;
    },
  ...
  const cancel = useMutation({
    ...
    onSuccess: () => {
      setCancelled(true);
      track("mp_queued_cancelled", { requestId: params.requestId });
  ...
  const delayed =
    v.delayed && (!keptDelay || cancelled || v.delayed.reversal)
      ? {
          ...
          onKeep: () => {
            setKeptDelay(true);
            track("mp_queued_kept_waiting", { requestId: params.requestId });
          },
