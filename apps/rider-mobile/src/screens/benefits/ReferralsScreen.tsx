          ...
          <>
            <Text variant="bodySm" tone="text2">
              ...
              <Text variant="bodySmStrong">
                {formatMinor(p.reward)} ride credit
              </Text>{" "}
              when a friend completes and pays for their first ride within 30
              days. They get {formatMinor(p.refereeBenefit)} off it. Up to{" "}
              {String(p.monthlyCap)} friends a month.
            </Text>
            <Card testID={TID.referrals.share.card} style={{ gap: 10 }}>
              <Pressable
                testID={TID.referrals.share.link}
                accessibilityRole="button"
                accessibilityLabel={"Your code " + p.code + ". Copy"}
                onPress={() => share("copy")}
                style={{
                  flexDirection: "row",
              ...
              <Text variant="caption" tone="text2">
                {p.url} · opens the app if installed, otherwise the store — your
                code carries over either way.
              </Text>
            </Card>
            <Text variant="label" tone="text3">
              Your referrals · {String(p.referrals.length)} ·{" "}
              {formatMinor(p.earnedTotal)} earned
            </Text>
            <Card
              testID={TID.referrals.status.list}
              ...
              {p.referrals.map((r, i) => (
                <Row key={r.id} last={i === p.referrals.length - 1}>
                  ...
                  >
                    <Text
                      variant="label"
                      tone={r.stage === "rewarded" ? "primaryInk" : "text2"}
                    >
                      {r.initials}
                    </Text>
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text variant="bodySm">{r.displayName}</Text>
                    <Text variant="caption" tone="text2">
                      {r.detail}
                    </Text>
                  </View>
