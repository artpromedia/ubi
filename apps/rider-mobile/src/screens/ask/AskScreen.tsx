...
type Block = {
  id: string;
  kind: "user" | "text" | "card" | "clarify" | "sources" | "review" | "refused";
  text?: string;
  card?: Card;
  fields?: ClarifyField[];
  sources?: Source[];
...
export function AskScreen() {
  ...
  useEffect(() => {
    if (!threadId)
      askApi
        .openThread("home")
        .then((r) => setThreadId(r.id))
        .catch(() =>
          setError(
            "Ask UBI is unavailable right now. You can still book with the forms.",
          ),
        );
    return () => stop.current?.();
  }, []);
  ...
  const send = (text: string, clarifications?: Record<string, unknown>) => {
    ...
    setStreaming(true);
    if (text.trim())
      setBlocks((b) => [...b, { id: "u" + Date.now(), kind: "user", text }]);
    setInput("");
    track("ask_message_sent", { threadId, chars: text.length });
    let current: Block | undefined;
    stop.current = askApi.stream(
      threadId,
      text,
      clarifications,
      (e: AskEvent) => {
        setBlocks((b) => {
          ...
          if (e.type === "token") {
            if (current && current.kind === "text") {
              current = { ...current, text: (current.text ?? "") + e.text };
          ...
          current = undefined;
          if (e.type === "card")
            next.push({ id: e.card.id, kind: "card", card: e.card });
          if (e.type === "clarify")
            next.push({
              id: "c" + Date.now(),
              kind: "clarify",
  ...
  const render = ({ item }: { item: Block }) => {
    switch (item.kind) {
      ...
      case "text":
        return <Text variant="bodySm">{item.text}</Text>;
      case "card":
        return <PlanCard card={item.card!} />;
      case "clarify":
        return (
          <ClarifyForm
            fields={item.fields!}
            passenger={{
              name: "Adaeze Nwosu",
              detail: "Name as on NIN · verified",
              initials: "AN",
            }}
            onSubmit={(a) => send("", a)}
          />
      ...
      case "sources":
        return (
          <AnswerSources
            ...
            onOpen={(s) => nav.navigate("PolicyDoc", { ref: s.ref })}
          />
        );
      case "review":
        ...
        return (
          <View style={{ flexDirection: "row", gap: 8 }}>
            <Button
              ...
              size="md"
              onPress={() => {
                track("ask_review_opened", { reviewId: item.reviewId });
                setReview(item.reviewId);
              }}
            />
            <Button
              testID={TID.ask.plan.editInForm}
              label="Edit in form"
              kind="secondary"
              size="md"
              onPress={() => {
                track("ask_edit_in_form", { target: "Travel.FlightSearch" });
      ...
      case "refused":
        return (
          <Banner
            tone="neutral"
            title="That isn't something Ask UBI can do"
            body="Use the regular screen for this — it opens with everything you need."
          />
        );
      default:
        return null;
  ...
  return (
    <Screen
      title="Ask UBI"
      subtitle="Answers about your account use UBI policies · prices are live"
      onBack={nav.goBack}
      action={{
        label: "Talk to a person",
        onPress: () => setHandoff(true),
        testID: TID.ask.handoff.start,
      }}
      scroll={false}
      bg="bg"
    >
      ...
      >
        <FlatList
          ...
          ListEmptyComponent={
            <View style={{ gap: 8 }}>
              ...
              {[
                "Plan Abuja Friday to Sunday, hotel near CBD",
                "Why was my ride credit reversed?",
                "Cheapest refundable flight to Abuja tomorrow",
              ].map((s) => (
                <Pressable
                  key={s}
        ...
        >
          <TextInput
            testID={TID.ask.composer.input}
            accessibilityLabel="Message Ask UBI"
            value={input}
          ...
          >
            <Text variant="heading" tone="onInverse">
              ↑
            </Text>
          </Pressable>
        </View>
