...
export function HandoffSheet({
  ...
  const start = async () => {
    ...
    setBusy(true);
    try {
      const r = await askApi.handoff(threadId, include);
      track("ask_handoff_started", { threadId, includeTranscript: include });
      onOpened(r.supportCaseId);
    } catch {
      setErr("Could not reach support chat. You can call instead.");
    } finally {
      setBusy(false);
    }
  };
  return (
    <Sheet
      visible={visible}
      onDismiss={onDismiss}
      testID={TID.ask.handoff.sheet}
    >
      <View style={{ gap: 12 }}>
        <Text variant="title">Talk to UBI Support</Text>
        <Text variant="bodySm" tone="text2">
          A person picks up where the assistant stopped. Support is open 24/7.
        </Text>
        <Card style={{ gap: 6 }}>
          ...
          <Text variant="bodySm">
            Your recent messages and plan cards, and any bookings or attempts
            made from this conversation.
          </Text>
          <Toggle
        ...
        <Text variant="caption" tone="text2">
          Your PIN, card and ID documents are never part of the conversation, so
        ...
        <Button
          ...
          label="Start chat with support"
          kind="inverse"
          loading={busy}
          onPress={start}
        />
        <Button
          label={"Call " + (config?.supportPhone ?? "support") + " instead"}
          kind="ghost"
          size="md"
          onPress={onDismiss}
        />
