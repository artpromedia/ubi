...
export function ItineraryScreen() {
  ...
  const q = useQuery({
    ...
    queryFn: () => travelApi.trip(params.tripId),
  });
  const act = (item: LinkedItem, key: string) => {
    if (key === "change" || key === "cancel")
      nav.navigate("Servicing", { orderId: item.orderId });
    else if (key === "retry_time" || key === "reserve")
      nav.navigate("AttachAirportRide", {
        orderId: item.orderId ?? params.tripId,
        direction: "from_airport",
      });
    else if (key === "boarding_pass")
      nav.navigate("BoardingPass", { orderId: item.orderId });
    else if (key === "add_return")
      nav.navigate("FlightSearch", { from: "ABV", to: "LOS" });
  ...
  return (
    <Screen
      title={t?.title ?? "Trip"}
      subtitle={t ? t.dates + " · all times WAT (UTC+1)" : undefined}
      onBack={nav.goBack}
