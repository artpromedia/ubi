...
export function StayRoomsScreen() {
  ...
  const q = useQuery({
    queryKey: ["rates", params.propertyId, params.searchId],
  ...
  return (
    ...
    >
      <View
        accessibilityLabel="Hotel photos, supplied by the property"
        style={{
          height: 170,
          marginHorizontal: -20,
      ...
      {!d ? (
        ...
        <>
          ...
          <Text variant="caption" tone="text2">
            {d.property.area} · {d.property.distanceKm} km from{" "}
            {d.property.distanceTo} · {d.property.checkIn} →{" "}
            {d.property.checkOut} · {d.property.nights} nights
          </Text>
          {d.rates.map((r) => (
            <RateCard
              key={r.id}
