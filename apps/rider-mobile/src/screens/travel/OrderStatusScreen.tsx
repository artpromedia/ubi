...
export function OrderStatusScreen() {
  ...
  React.useEffect(() => {
    if (o)
      track("travel_order_status_viewed", { orderId: o.id, state: o.state });
  }, [o?.state]);
  return (
    <Screen
      onBack={nav.goBack}
      footer={
        o ? (
          <Button
            label={
              o.state === "ticketed" || o.state === "confirmed"
                ? "View itinerary"
                : "Notify me · go back home"
            }
            kind="inverse"
            onPress={() =>
              o.state === "ticketed" || o.state === "confirmed"
                ? nav.navigate("Itinerary", { tripId: o.tripId })
    ...
    >
      {!o ? (
        ...
        <>
          ...
          <Card>
            <Ladder
              testID={TID.travel.order.ladder}
              steps={o.ladder.map((s) => ({
                label: s.label,
                detail: s.detail,
                state: s.state,
              }))}
          ...
          {o.state === "unknown_reconciling" ? (
            <Banner
              tone="warn"
              title="We're confirming with the supplier"
              body="The answer took too long. We check the original request — we never book it twice. Your money stays held until we know."
            />
          ) : null}
        </>
      )}
