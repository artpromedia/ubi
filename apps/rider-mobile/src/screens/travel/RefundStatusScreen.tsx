...
export function RefundStatusScreen() {
  ...
  const { params } =
    useRoute<RouteProp<TravelStackParamList, "RefundStatus">>();
  const q = useQuery({
    queryKey: ["refund", params.refundId],
    queryFn: () => travelApi.refund(params.refundId),
    refetchInterval: (query) =>
      query.state.data?.stage === "refunded_to_wallet" ||
      query.state.data?.stage === "rejected"
        ? false
        : 60_000,
  });
  const r = q.data;
        ...
        <View style={{ flexDirection: "row", gap: 8 }}>
          <Button
            ...
            onPress={() =>
              nav.navigate("Ask", {
                screen: "Thread",
                params: { seed: "Where is my refund?" },
              })
            }
          />
          <Button
    ...
    >
      {!r ? (
        ...
        <>
          ...
          <Card>
            <Ladder
              testID={TID.travel.refund.tracker}
              steps={r.steps.map((s) => ({
                label: s.label,
                detail: s.detail,
                state: s.state,
