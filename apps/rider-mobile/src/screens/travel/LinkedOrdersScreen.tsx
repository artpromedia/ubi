...
export function LinkedOrdersScreen() {
  ...
  const { params } =
    useRoute<RouteProp<TravelStackParamList, "LinkedOrders">>();
  const q = useQuery({
    queryKey: ["linked", params.tripId],
    queryFn: () => travelApi.linked(params.tripId),
  });
  const t = q.data;
  const failedRide = t?.items.find(
    (i) => i.kind === "ride_reservation" && i.status === "not_reserved",
  );
  const act = (item: LinkedItem, key: string) =>
    nav.navigate(key === "book_on_day" ? "Main" : "AttachAirportRide", {
  ...
  return (
    <Screen
      ...
      footer={
        failedRide ? (
          <Button
            label="Reserve a different class or time"
            kind="inverse"
            onPress={() =>
              nav.navigate("AttachAirportRide", {
                orderId: failedRide.orderId,
                direction: "to_airport",
              })
            }
          />
      ...
      {!t ? (
        ...
        <>
          ...
          <Text variant="bodySm" tone="text2">
            These are separate orders. Here is what happened to each and what it
            costs you.
          </Text>
          {t.items.map((it) => (
