...
export function AlternativeCard({
  ...
  return (
    <Pressable
      testID={TID.travel.disruption.alternative}
      ...
      <Card emphasis={selected} style={{ gap: 3 }}>
        ...
        >
          <Text variant="heading" tabular>
            {hm(a.departAt)} → {hm(a.arriveAt)}
          </Text>
          {free ? (
            <Text variant="bodySmStrong" tone="primaryInk">
              ₦0 to you
            </Text>
          ) : (
            <MoneyText money={a.customerPays} variant="bodySmStrong" />
          )}
        </View>
        <Text variant="caption" tone="text2">
          {a.carrier} {a.flightNumber}
          {a.fareFamily ? " · " + a.fareFamily : ""}
          {a.baggage ? " · " + a.baggage : ""}
          {a.seatsLeft !== undefined ? " · " + a.seatsLeft + " seats left" : ""}
        </Text>
        {a.covered && a.covered.amountMinor > 0 ? (
          <Text variant="caption" tone="text2">
            Fare {formatMinor(a.price)} · UBI covers {formatMinor(a.covered)}{" "}
            difference
            {a.heldUntil
              ? " · seat is held for you until " + hm(a.heldUntil)
              : ""}
          </Text>
        ) : null}
        {a.note ? (
