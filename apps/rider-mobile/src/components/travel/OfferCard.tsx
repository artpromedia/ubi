...
export function FareFamilyOption({
  f,
  selected,
  onPress,
}: {
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={
        f.name + ", " + f.baggage + ", " + f.changeRule + ", " + f.refundRule
      }
      onPress={onPress}
      style={{
        flex: 1,
    ...
    >
      <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
        <Text variant="bodySmStrong">{f.name}</Text>
        <MoneyText money={f.price} variant="bodySmStrong" />
      </View>
...
export function OfferCard({
  ...
  return (
    <Card
      testID={TID.flights.results.offer}
      emphasis={emphasis}
      style={{ gap: 8, opacity: offer.soldOut ? 0.7 : 1 }}
    >
      <View
        style={{
          flexDirection: "row",
      ...
      {offer.soldOut ? (
        ...
        <>
          <View style={{ flexDirection: "row", gap: 8 }}>
            {offer.fareFamilies.map((ff) => (
              <FareFamilyOption
                key={ff.id}
                f={ff}
                selected={selectedFamily === ff.id}
                onPress={() => onSelectFamily(ff.id)}
              />
            ))}
          </View>
          {g ? (
            ...
            >
              <StatusPill
                status="live"
                suffix={"price guaranteed to " + hm(g)}
              />
              <Text variant="caption" tone="text2">
                if you book now · not a seat reservation
              </Text>
            </View>
          ) : (
