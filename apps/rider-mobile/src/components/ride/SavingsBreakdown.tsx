...
export function SavingsBreakdown({
  ...
  return (
    <View
      testID={TID.rider.quote.savings}
      style={{
        borderTopWidth: 1,
    ...
    >
      {row(
        <Text variant="caption">Fare</Text>,
        <MoneyText money={fare} variant="caption" />,
      )}
      {adjustments.map((a) =>
        a.reasonCode
          ? row(
              ...
              <Text variant="caption" tone="text3">
                {a.reasonCode === "budget_exhausted"
                  ? "budget used up"
                  : a.reasonCode === "min_spend_not_met"
                    ? "min spend not met"
          ...
          : a.type === "fee_waiver"
            ? row(
                ...
                <Text variant="caption">
                  <MoneyText
                    ...
                    struck
                  />{" "}
                  <Text variant="caption" tone="primaryInk">
                    waived
                  </Text>
                </Text>,
              )
            : row(
                ...
                <MoneyText
                  money={{
                    amountMinor: -Math.abs(a.amount.amountMinor),
      ...
      {row(
        <Text variant="bodySmStrong">You pay</Text>,
        <Text variant="bodySmStrong">
          <MoneyText money={payable} variant="bodySmStrong" /> · {paymentLabel}
        </Text>,
      )}
      {note ? (
        <Text variant="caption" tone="text2" style={{ marginTop: 6 }}>
          {note}
        </Text>
      ) : null}
    </View>
...
export function SavingsChangedBanner({ text }: { text: string }) {
  return (
    <Banner testID={TID.rider.quote.savingsChanged} tone="warn" body={text} />
  );
}
