...
export function QuoteCard({ card }: { card: CardDto }) {
  ...
  return (
    <View
      testID={TID.ask.plan.card}
      accessibilityLabel={
        card.title +
        ", " +
        (card.status === "live" ? "live price" : card.status)
      }
      style={{
        borderWidth: 1,
    ...
    >
      <View
        style={{
          backgroundColor:
            card.status === "expired" ? t.colors.bg2 : t.colors.travelTint,
      ...
      >
        <StatusPill
          status={card.status === "live" ? "live" : "expired"}
          suffix={card.status === "live" ? ageLabel(card.quotedAt) : undefined}
        />
        <Text variant="caption" tone="travelInk" style={{ flex: 1 }}>
          {card.subtitle}
        </Text>
        {card.price ? (
          <MoneyText money={card.price} variant="bodySmStrong" />
        ) : null}
      </View>
      <View style={{ padding: 12, gap: 4, backgroundColor: t.colors.card }}>
        <Text variant="bodyStrong">{card.title}</Text>
        {card.warnings?.map((w) => (
          <Text key={w} variant="caption" tone="warnInk">
            {w}
          </Text>
        ))}
      </View>
...
export function PlanCard({ card }: { card: CardDto }) {
  return card.status === "suggestion" ? (
