...
export function ClarifyForm({
  ...
  fields: ClarifyField[];
  passenger?: { name: string; detail: string; initials: string };
  ...
  const complete = fields.every(
    (f) =>
      !f.required || answers[f.key] !== undefined || f.kind === "passenger",
  );
  return (
    <Card testID={TID.ask.clarify.form} style={{ gap: 10 }}>
      {fields.map((f) => (
        ...
        >
          <Text variant="label" tone="text2" style={{ marginBottom: 8 }}>
            {f.label}
          </Text>
          {f.kind === "chips" ? (
          ...
          {f.kind === "passenger" && passenger ? (
            ...
            >
              ...
              >
                <Text variant="label" tone="primaryInk">
                  {passenger.initials}
                </Text>
              </View>
              <View style={{ flex: 1 }}>
                <Text variant="bodySm">{passenger.name}</Text>
