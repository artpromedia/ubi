// Caller-held Idempotency-Keys for the rider's trip, route and booking state POSTs
// (launch CLAUDE.md #3). api() mints a fresh key per call, which would turn a retry after
// a dropped response into a second command. Instead each distinct command (its
// fingerprint: action + ids + the exact versions/revisions it binds to) keeps ONE key
// until the server gives a definite answer — success or a 4xx refusal. A network failure
// or a 5xx keeps the key, so the retry replays the server's first outcome rather than
// acting twice. Same pattern as the driver app's lib/idempotency.ts.
