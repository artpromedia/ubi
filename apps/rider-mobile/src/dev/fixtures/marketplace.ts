...
const qparam = (path: string, key: string) => {
...
type MpFixState = {
  ...
  awardPolls: number;
  queuePolls: number;
  queueCancelPolls: number;
  publishedAt: string;
  selected: "bid_emeka" | "bid_chidi" | null;
  cancelled: boolean;
...
const quoteEnvelope = (service: string, vehicleClass: string) => ({
  ...
  policyVersion: 3,
  breakdown: [
    { label: "Base", amountMinor: NGN(700) },
    { label: "Distance · 8.4 km", amountMinor: NGN(1400) },
    { label: "Time · 22 min", amountMinor: NGN(500) },
  ],
  routedDistanceMeters: 8400,
  routedDurationSec: 1320,
});
...
const driver = (
  ...
  vehicle: string,
  plateMasked: string,
) => ({ displayName, initials, rating, completedTrips, vehicle, plateMasked });
...
const offerChidi = () => ({
  ...
  pickupLabel: "Pickup window 12–18 min",
  pickupWindow: { earliestSec: 720, latestSec: 1080, etaVersion: 2 },
  expiresAt: iso(110_000),
  withdrawn: false,
  whyRecommended:
    "Finishing a trip 1.4 km from your pickup · 4.8★ over 1,876 trips · offers your price",
  bookingFeeMinor: NGN(200),
  totalMinor: NGN(3000),
  deltaLabel: null,
});
const offerTunde = (withdrawn: boolean) => ({
  ...
  pickupLabel: "Pickup in 3 min · 0.9 km away",
  pickupWindow: null,
  expiresAt: iso(75_000),
  withdrawn,
  whyRecommended: null,
  bookingFeeMinor: NGN(200),
  totalMinor: NGN(3200),
  deltaLabel: "+₦200 above your ask",
});

const requestBody = (
  id: string,
  state: string,
  requested: { amountMinor: number; currency: string },
  opts: { step?: number; closeReason?: string | null; version?: number } = {},
) => ({
  requestId: id,
  state,
  revision: 1,
  ...
  searchEnvelope: {
    step: opts.step ?? 0,
    radiusMeters: (opts.step ?? 0) > 0 ? 5000 : 3000,
    pickupEtaSec: 600,
  },
  policyVersion: 3,
  pricingVersion: "px_2026_09_1",
...
const award = (
  ...
  requestVersion: 1,
  bidVersion: 1,
  driverId: bidId === "bid_chidi" ? "u_drv_chidi" : "u_drv_emeka",
  requesterId: "u_rider_1",
  fareMinor: NGN(2800),
  commissionMinor: NGN(280),
  slot, // 10% of ₦2,800, computed server-side (commissionMinorFor)
  executionRef:
    state === "confirmed" ? { service: "ride", id: "ride_mp_901" } : null,
  createdAt: st().publishedAt,
...
export async function marketplaceFixtures(i: FixtureInput) {
  ...
  if (i.method === "GET" && i.path.startsWith("/v1/mp/quote")) {
    return ok(
      quoteEnvelope(
        qparam(i.path, "service") ?? "ride",
        qparam(i.path, "vehicleClass") ?? "standard",
      ),
    );
  }
  if (i.method === "POST" && i.path === "/v1/mp/requests") {
    const asked =
      (
        i.body as {
          requestedFareMinor?: { amountMinor: number; currency: string };
        }
      )?.requestedFareMinor ?? NGN(2800);
    if (asked.amountMinor < 2400_00)
      return {
        status: 422,
        json: {
          code: "fare_out_of_bounds",
          message: "Below the minimum for this route. Enter at least ₦2,400.",
          details: { field: "requestedFareMinor", minimumFareMinor: NGN(2400) },
        },
      };
    globalThis.__ubiMpFix = fresh();
    // Publishing at exactly the floor demonstrates the R07a no-offers branch.
    if (asked.amountMinor === 2400_00)
      return { status: 201, json: requestBody("req_mp_low", "open", asked) };
  ...
  if (i.method === "GET" && /^\/v1\/mp\/requests\/req_mp_low$/.test(i.path)) {
    ...
    if (s.lowPolls >= 3)
      return ok({
        request: requestBody("req_mp_low", "no_offers", NGN(2400), {
    ...
    return ok({
      request: requestBody("req_mp_low", "open", NGN(2400)),
      offers: [],
      seq: s.lowPolls,
    });
  }
  if (i.method === "GET" && /^\/v1\/mp\/requests\/req_mp_1$/.test(i.path)) {
    ...
    const offers =
      s.polls <= 1
        ...
        : s.polls === 2
          ? [offerEmeka()]
          : [offerEmeka(), offerChidi(), offerTunde(s.polls >= 4)];
    const state = s.cancelled
      ? "cancelled"
      : s.selected
        ? s.awardPolls >= 2
          ? "awarded"
    ...
    const aw = s.selected
      ? award(
          s.awardPolls >= 2 ? "confirmed" : "pending",
          s.selected === "bid_chidi" ? "next" : "current",
          s.selected,
        )
      : undefined;
    // Step 1 envelope widening (R09) once the finishing-trip offer is in play; existing bids kept.
    return ok({
      request: requestBody("req_mp_1", state, NGN(2800), {
        step: s.polls >= 3 ? 1 : 0,
        closeReason: s.cancelled ? "cancelled" : null,
      }),
      offers,
      ...(aw ? { award: aw } : {}),
      seq: s.polls,
  ...
  if (
    i.method === "POST" &&
    /^\/v1\/mp\/requests\/req_mp_1\/select$/.test(i.path)
  ) {
    ...
    if (bidId === "bid_tunde")
      return {
        ...
        json: {
          ...
          message:
            "Tunde withdrew this offer while you were deciding. Offers below are refreshed — nothing was charged. Pick again.",
        },
      };
  ...
  if (
    i.method === "GET" &&
    /^\/v1\/mp\/requests\/req_mp_1\/award$/.test(i.path)
  ) {
    if (!s.selected)
      return {
        status: 409,
        json: {
          ...
          message: "No selection has been made on this request.",
        },
      };
    s.awardPolls += 1;
    return ok(
  ...
  if (
    i.method === "POST" &&
    /^\/v1\/mp\/requests\/req_mp_(1|low)\/cancel$/.test(i.path)
  ) {
    if (s.selected) {
      s.queueCancelled = true;
      s.queueCancelPolls = 0;
    }
    s.cancelled = true;
    return ok(
      requestBody(
        i.path.includes("req_mp_low") ? "req_mp_low" : "req_mp_1",
        "cancelled",
        NGN(2800),
  ...
  if (
    i.method === "POST" &&
    /^\/v1\/mp\/requests\/req_mp_1\/revise$/.test(i.path)
  ) {
    const b = i.body as {
      requestedFareMinor: { amountMinor: number; currency: string };
    };
    ...
    if (s.queueCancelled) {
      ...
      return ok({
        ...
        delayed: {
          ...
          noticeBody:
            "The new estimate was outside the window you accepted, so this cancellation costs nothing. Money movements below settle as the events confirm.",
          keepLabel: "Search again instead",
          reversal: {
            riderHold: settled ? "released" : "releasing",
            driverFee: settled ? "reversed" : "pending",
          },
        },
      });
    }
    if (s.queuePolls >= 3) {
      return ok({
        ...
        steps: [
          {
            ...
            detail: "Fare fixed at ₦2,800",
            state: "done",
          },
          {
            label: "Finishing their current trip",
            detail: "Now estimated 24 min",
            state: "active",
          },
        ...
        delayed: {
          ...
          noticeBody:
            "You accepted a pickup window of up to 18 min. You can keep waiting, or cancel free and search again — your money is only moved when a trip happens.",
          keepLabel: "Keep waiting · new ETA 24 min",
          reversal: null,
        },
      });
    }
