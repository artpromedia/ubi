...
export type Mandate = MandateInput & {
  ...
  usage: { amountUsed: Money; runsUsed: number; periodStart: string };
  lastRunAt?: string;
  summary: string;
...
export const mandatesApi = {
  ...
  create: (input: MandateInput, proof: string) =>
    api<Mandate>("POST", "/v1/mandates", {
      ...input,
  ...
  patch: (
    ...
    api<Mandate>("PATCH", "/v1/mandates/" + id, {
      op,
      changes,
      assurance: proof ? { method: "pin", proof } : undefined,
    }),
  executions: (id: string) =>
    api<MandateExecution[]>("GET", "/v1/mandates/" + id + "/executions"),
