...
export type ReviewItem = {
  kind: "flight" | "stay" | "ride_reservation";
  title: string;
  detail?: string;
  price: Money;
...
export type ExecutionItem = {
  kind: "payment" | "flight" | "stay" | "ride_reservation";
  title: string;
...
export const askApi = {
  ...
  confirmReview: (reviewId: string, termsVersion: string, proof: string) =>
    api<{ executionId: string }>(
      "POST",
      "/v1/ask/reviews/" + reviewId + "/confirm",
      { termsVersion, assurance: { method: "pin", proof } },
    ),
