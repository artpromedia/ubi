 ...
 */
emailRoutes.post('/send', serviceAuth, zValidator('json', sendEmailSchema), async (c) => {
  ...
  if (data.userId) {
    ...
    if (prefs && !prefs.emailEnabled) {
      return c.json({
        success: false,
        error: { code: 'EMAIL_DISABLED', message: 'User has disabled email notifications' },
      }, 400);
    }
  }
 ...
 */
emailRoutes.post('/batch', serviceAuth, zValidator('json', sendBatchEmailSchema), async (c) => {
 ...
 */
emailRoutes.post('/transactional', serviceAuth, zValidator('json', sendTransactionalSchema), async (c) => {
  ...
  const result = await emailService.send({
    ...
    html: interpolateTemplate(template.htmlBody || template.body, templateData),
  ...
  return c.json({
    ...
    error: result.success ? undefined : { code: 'EMAIL_FAILED', message: result.error },
...
emailRoutes.post('/receipt', serviceAuth, async (c) => {
  const { userId, orderId, orderType, amount, currency, items } = await c.req.json<{
    userId: string;
    orderId: string;
  ...
  if (prefs && !prefs.emailReceipts) {
    return c.json({ success: true, data: { skipped: true, reason: 'User disabled receipts' } });
  }
