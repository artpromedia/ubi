 ...
 */
smsRoutes.post('/send', serviceAuth, zValidator('json', sendSMSSchema), async (c) => {
  ...
  if (userId) {
    ...
    if (prefs && !prefs.smsEnabled) {
      return c.json({
        success: false,
        error: { code: 'SMS_DISABLED', message: 'User has disabled SMS notifications' },
      }, 400);
    }
  }
  ...
  if (!result.success) {
    return c.json({
      success: false,
      error: { code: 'SMS_FAILED', message: result.error || 'Failed to send SMS' },
    }, 500);
  }
 ...
 */
smsRoutes.post('/batch', serviceAuth, zValidator('json', sendBatchSMSSchema), async (c) => {
 ...
 */
smsRoutes.post('/otp', serviceAuth, zValidator('json', sendOTPSchema), async (c) => {
  const { userId, phone, purpose, length, expiresInMinutes } = c.req.valid('json');
 ...
 */
smsRoutes.post('/otp/verify', zValidator('json', verifyOTPSchema), async (c) => {
...
function getOTPMessage(purpose: string, otp: string, expiresIn: number): string {
  ...
  return messages[purpose] || `Your UBI code is ${otp}. Valid for ${expiresIn} minutes.`;
}
