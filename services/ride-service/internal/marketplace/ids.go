...
// the same shape services/config-service/src/lib/ids.ts writes.
...
// creates, so a replay collides with its own earlier write.
func deterministicID(prefix string, parts ...string) string {
	return prefix + "_" + digest(strings.Join(parts, "|"))
}

// itoa keeps event idempotency keys free of fmt formatting.
