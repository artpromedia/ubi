 ...
 * server is authoritative). In particular, the *author* of a campaign and the
 * *approver* of it are both taken from the gateway identity, which is what makes
 * the two-person check trustworthy.
 */
import {
  ContractError,
  IDEMPOTENCY_HEADER,
...
export async function gatewayAuth(
  ...
  if (role === undefined || !isKnownRole(role)) {
    return c.json(
      {
        code: "forbidden",
        message: "this role has no access to the growth service",
...
/**
 * The city the request belongs to. It arrives as a gateway header. Optional for
 * some admin reads that span cities; required where a flag must be evaluated.
 */
...
export function optionalCityOf(c: Context): string | null {
  const cityId = c.req.header("X-City-ID");
  return cityId === undefined || cityId.length === 0 ? null : cityId;
}
