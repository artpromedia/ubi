...
describe("signed internal identity context", () => {
  it("round-trips every field", async () => {
    const verified = await verifyIdentityContext(await signIdentityContext(BASE));
  ...
  it("carries the key id, issuer and audience so a consumer can pin them", async () => {
    ...
    expect(decodeSegment(token, 0)).toMatchObject({ alg: "HS256", kid: "test-k1", typ: "UBI-IC" });
  ...
  it("rejects a tampered payload", async () => {
    ...
    await expect(
      verifyIdentityContext(`${header as string}.${forged}.${signature as string}`),
    ).rejects.toMatchObject({ code: "unauthorized" });
  });

  it("rejects a context signed with a different key", async () => {
    const token = await signIdentityContext(BASE);
    process.env.UBI_IDENTITY_SECRET = "a-completely-different-internal-secret-9";
    await expect(verifyIdentityContext(token)).rejects.toMatchObject({ code: "unauthorized" });
  });
  ...
  it("rejects a context signed with the client-facing JWT secret", async () => {
    process.env.UBI_IDENTITY_SECRET = "an-interim-internal-secret-for-signing-1";
    const token = await signIdentityContext(BASE);

    process.env.UBI_IDENTITY_SECRET = process.env.JWT_SECRET;
    await expect(verifyIdentityContext(token)).rejects.toThrow(/must not equal JWT_SECRET/);
    await expect(signIdentityContext(BASE)).rejects.toThrow(/must not equal JWT_SECRET/);
  });

  it("refuses a signing secret that is too short to be a key", async () => {
    process.env.UBI_IDENTITY_SECRET = "short";
    await expect(signIdentityContext(BASE)).rejects.toThrow(/at least 32 characters/);
  });
  ...
  it("still verifies contexts signed with the previous key during a rotation", async () => {
    ...
    process.env.UBI_IDENTITY_SECRET = "the-next-internal-identity-secret-000001";
  ...
  it("drops unknown scopes and modes rather than passing them through", async () => {
    const token = await signIdentityContext({
      ...BASE,
      scopes: ["ride:read", "not:a:real:scope"] as unknown as IdentityContext["scopes"],
