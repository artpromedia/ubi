 ...
 *
 * EVERY fleet-service client route reads its caller from this context
 * (middleware/auth.ts): who is proposing terms, reporting a vehicle off-road,
 * signing an offer or changing their availability must be what the gateway
 * proved — never a plain header anyone on the service network could set. PIN
 * signing raises the stakes: fleet-service RELAYS this context, unchanged, to
 * user-service's wallet-PIN check (./identity-relay.ts), so an unverified
 * caller must never reach that relay. This is travel-service's verifier (the
 * port of ask-service's); payment-service and user-service verify the same
 * claims with the same secret.
 *
 ...
 * the INTERNAL secret UBI_IDENTITY_SECRET (services/api-gateway/src/identity/
 * context.ts is the issuer).
 *
 ...
 * called by index.ts before anything else).
 */
...
export function assertIdentityConfigured(
  ...
  try {
    ...
    if (isProductionEnvironment(env.NODE_ENV)) {
      throw new Error(
        `${error instanceof Error ? error.message : String(error)}: in production fleet-service authenticates every client route with the gateway-signed identity context and never falls back to plain headers`,
      );
    }
    return false;
  }
