 ...
 * restriction modes are active (mod) and WHERE it is scoped (city). It signs it
 * with the INTERNAL secret UBI_IDENTITY_SECRET — never a key a client holds —
 * and forwards it with every proxied request
 * (services/api-gateway/src/identity/context.ts is the issuer; user-service and
 * payment-service verify the same claims with the same secret).
 *
 * This is the same verification, without a JOSE dependency: the header must
 * say HS256 (anything else — `none` included — is refused), the HMAC over
 * `<header>.<payload>` must match one configured key in constant time, and the
 * issuer, audience and expiry must hold. Every failure is the same
...
function keyFrom(env: NodeJS.ProcessEnv, name: string): Buffer | undefined {
