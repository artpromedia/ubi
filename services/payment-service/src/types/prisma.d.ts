...
declare module '@prisma/client' {
  ...
  export class PrismaClient {
    ...
    $queryRaw<T = any>(query: TemplateStringsArray, ...values: unknown[]): Promise<T>;
    $queryRawUnsafe<T = any>(query: string, ...values: unknown[]): Promise<T>;
    $transaction<T>(fn: (prisma: Omit<PrismaClient, '$connect' | '$disconnect' | '$transaction'>) => Promise<T>): Promise<T>;
