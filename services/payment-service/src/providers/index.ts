...
export const PROVIDER_CAPABILITIES: Record<PaymentProvider, ProviderCapabilities> = {
  ...
  flutterwave: {
    currencies: [Currency.NGN, Currency.GHS, Currency.KES, Currency.UGX, Currency.TZS, Currency.ZAR, Currency.XOF, Currency.USD],
 ...
 */
export function getBestProvider(currency: Currency, country?: string): PaymentProvider {
  ...
  if ([Currency.KES, Currency.UGX, Currency.TZS, Currency.XOF].includes(currency)) {
    return 'flutterwave';
  }
 ...
 */
export function createProviderClient(provider: PaymentProvider): PaystackClient | FlutterwaveClient {
  switch (provider) {
    case 'paystack':
