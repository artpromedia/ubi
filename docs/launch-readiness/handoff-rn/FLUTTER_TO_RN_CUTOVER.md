...
**Decision:** UBI Rider and UBI Driver are React Native TypeScript apps. Flutter (`mobile/`) is a _reference implementation_ from today until RN acceptance, then it is removed. No new Flutter work is accepted from the moment RN-01 starts. This page tells Claude Code exactly what to change, in what order, and what "Flutter is gone" means.
