...
Rides: POST /v1/rides/quotes → POST /v1/rides (Idempotency-Key) → GET /v1/rides/:id · stream `ride.*` events with seq → replay. Wallet: /v1/wallet/_ (slice 04). Auth: /v1/auth/_ + device enrolment (slice 03). Reconnect: exponential backoff 1–30 s, jitter, lastSeq; offline banner `rider.offline.banner` with stale timestamp.
