...
type Options<S> = {
  channel: string;
  snapshotPath: string;
  reduce: (state: S | undefined, e: Envelope) => S;
  wsUrl?: string;
};
...
export function useResumableStream<S>(opts: Options<S>) {
  ...
  useEffect(() => {
    ...
    const snapshot = async () => {
      const snap = await api<{ seq: number; state: S }>(
        "GET",
        opts.snapshotPath,
      );
    ...
    const connect = async () => {
      ...
      ws = new WebSocket(
        (opts.wsUrl ?? "wss://rt.ubi.africa/v1/stream") +
          "?channel=" +
          encodeURIComponent(opts.channel) +
          "&lastSeq=" +
          lastSeq.current +
          "&token=" +
          (token ?? ""),
      ...
      ws.onmessage = (m) => {
        ...
        if ((e as { type: string }).type === "replay_overflow") {
          void snapshot();
          return;
        }
      ...
      ws.onclose = () => {
        ...
        timer = setTimeout(connect, backoff.current + Math.random() * 500);
