...
type Ctx = {
  flags: FlagSet & Partial<Record<NewFlagKey, boolean>>;
...
export function FlagsProvider({
  ...
  const [state, setState] = useState<Omit<Ctx, "refresh">>({
    flags: DENY_ALL,
    status: "loading",
  });
  const refresh = async () => {
    if (!cityId) {
      setState({ flags: DENY_ALL, status: "denied" });
      return;
    }
    try {
      const flags = await api<Record<string, boolean>>(
        "GET",
        "/v1/config/flags?cityId=" + encodeURIComponent(cityId),
      );
...
export function useFlag(key: AnyFlag): boolean {
  ...
  return (
    isEnabled(flags as FlagSet, key as FlagKey) ||
