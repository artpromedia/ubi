...
export type StatusKey = 'suggestion' | 'live' | 'awaiting_confirmation' | 'processing' | 'supplier_pending' | 'confirmed' | 'ticketed' | 'reserved' | 'assigned' | 'completed' | 'failed' | 'expired' | 'partly_booked' | 'blocked' | 'done' | 'active' | 'paused' | 'revoked' | 'reversed' | 'earned' | 'used_up' | 'scheduled' | 'not_reserved' | 'not_booked' | 'in_review' | 'qualifying' | 'rewarded' | 'refund_in_progress' | 'refunded' | 'cancelled';
const LABEL: Record<StatusKey, string> = {
  suggestion: 'Suggestion', live: 'Live price', awaiting_confirmation: 'Awaiting your confirmation', processing: 'Processing', supplier_pending: 'Supplier pending', confirmed: 'Confirmed', ticketed: 'Ticketed', reserved: 'Reserved', assigned: 'Assigned', completed: 'Completed', failed: 'Failed', expired: 'Expired', partly_booked: 'Partly booked', blocked: 'Blocked', done: 'Done', active: 'Active', paused: 'Paused', revoked: 'Revoked', reversed: 'Reversed', earned: 'Earned', used_up: 'Used up', scheduled: 'Scheduled', not_reserved: 'Not reserved', not_booked: 'Not booked', in_review: 'In review', qualifying: 'Qualifying', rewarded: 'Rewarded', refund_in_progress: 'Refund in progress', refunded: 'Refunded', cancelled: 'Cancelled',
};
export function StatusPill({ status, suffix, testID }: { status: StatusKey; suffix?: string; testID?: string }) {
  ...
  const tone = (): { bg: string; fg: string; dashed?: boolean } => {
    switch (status) {
      ...
      case 'confirmed': case 'ticketed': case 'reserved': case 'assigned': case 'completed': case 'done': case 'active': case 'earned': case 'rewarded': return { bg: t.colors.okTint, fg: t.colors.ok };
      ...
      case 'refunded': case 'not_booked': return { bg: t.colors.bg2, fg: t.colors.text2 };
