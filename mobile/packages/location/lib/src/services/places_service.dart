...
class PlacesService {
  ...
  Future<Result<List<PlaceSearchResult>>> searchPlaces({
    ...
    try {
      ...
      if (response.statusCode != 200) {
        return Result.failure(ServerFailure(message: 'Places API error: ${response.statusCode}'));
      }
      ...
      if (status != 'OK' && status != 'ZERO_RESULTS') {
        return Result.failure(ServerFailure(message: 'Places API error: $status'));
      }
      ...
      return Result.failure(ServerFailure(message: 'Place search failed: $e'));
    }
  }
  ...
  Future<Result<List<PlaceSearchResult>>> autocompletePlaces({
    ...
    try {
      ...
      if (response.statusCode != 200) {
        return Result.failure(ServerFailure(message: 'Autocomplete API error: ${response.statusCode}'));
      }
      ...
      if (status != 'OK' && status != 'ZERO_RESULTS') {
        return Result.failure(ServerFailure(message: 'Autocomplete API error: $status'));
      }
      ...
      return Result.failure(ServerFailure(message: 'Autocomplete failed: $e'));
    }
  }
  ...
  Future<Result<PlaceDetails>> getPlaceDetails(String placeId) async {
    ...
    try {
      ...
      if (response.statusCode != 200) {
        return Result.failure(ServerFailure(message: 'Place details API error: ${response.statusCode}'));
      }
      ...
      if (status != 'OK') {
        return Result.failure(ServerFailure(message: 'Place details API error: $status'));
      }

      final result = data['result'] as Map<String, dynamic>;
      return Result.success(_parsePlaceDetails(result));
    } catch (e) {
      return Result.failure(ServerFailure(message: 'Get place details failed: $e'));
    }
  }
  ...
  Future<Result<List<PlaceSearchResult>>> getNearbyPlaces({
    ...
    try {
      ...
      if (response.statusCode != 200) {
        return Result.failure(ServerFailure(message: 'Nearby search API error: ${response.statusCode}'));
      }
      ...
      if (status != 'OK' && status != 'ZERO_RESULTS') {
        return Result.failure(ServerFailure(message: 'Nearby search API error: $status'));
      }
      ...
      return Result.failure(ServerFailure(message: 'Nearby search failed: $e'));
    }
  }
